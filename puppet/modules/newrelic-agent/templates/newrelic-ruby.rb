...
common: &default_settings
  ...
  # New Relic service.
<% if (['prod-2a','admin']).include? @hostname then -%>
  license_key: '11d32906468858915350f2b'
<% else -%>
  license_key: '8160b3303ee260f7c6bfa'
<% end -%>
  ...
  # If you are using Unicorn, make sure that you have configured it to
  # use preload_app: true - the agent may not start in other modes.
  preload_app: true
...
production:
  <<: *default_settings
  monitor_mode: true
<% if (['prod-2a','prod-2b']).include? @hostname then -%>
  app_name: prod_<%= app_name %>
<% elsif (['prod-2c','prod-2d']).include? @hostname then -%>
  app_name: resque
<% end -%>
  agent_enabled: auto
...
qa:
  <<: *default_settings
  monitor_mode: true
<% if (['qa2','qa1']).include? @hostname then -%>
  app_name: community_qa
<% elsif (['staging1']).include? @hostname then -%>
  app_name: qa_<%= app_name %>_app
<% end -%>
  agent_enabled: auto
